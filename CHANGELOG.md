...
- **Web: Whisperer server-side estimate now preserves uploaded file context.** The estimate-page flow keeps the upload token across steps so users can proceed to transcription without re-uploading and without "No file selected" errors. Added additional submit-path hardening so expired or invalid upload sessions return a user-friendly form error instead of a generic 500 page.
