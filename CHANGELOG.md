...
- **PDF forms v1 Slice 1 implementation (beta, flag-gated)**: added a new core inspector in `desktop/src/acb_large_print/pdf_forms.py` that classifies PDF round-trip eligibility (AcroForm supported, weak metadata, XFA unsupported, static/flattened, signature-blocked), extracts field metadata with inferred labels/confidence/warnings, and returns structured summary counts; added CLI command `acb-large-print pdf-inspect` in `desktop/src/acb_large_print/cli.py`; added web beta routes in `web/src/acb_large_print_web/routes/pdf_forms.py` (`GET /pdf-forms/`, `POST /pdf-forms/inspect`, `POST /pdf-forms/api/inspect`) with template `web/src/acb_large_print_web/templates/pdf_forms_form.html`; wired route registration and feature context in `web/src/acb_large_print_web/app.py`; added new feature flag `GLOW_ENABLE_PDF_FORM_ROUNDTRIP_BETA` (default `false`) in `web/src/acb_large_print_web/feature_flags.py`; and added regression tests in `desktop/tests/test_pdf_forms.py`, `desktop/tests/test_cli_pdf_inspect.py`, and `web/tests/test_pdf_forms_routes.py`.
