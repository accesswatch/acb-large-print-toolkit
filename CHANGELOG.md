...
- **Web: Whisperer server-side estimate fallback step.** Added a no-JavaScript fallback button, "Next: calculate estimate on server", that posts to a new `/whisperer/estimate-page` route, computes estimate server-side, and re-renders the same form with populated estimate values so users can proceed even when client-side button events fail.
- **Web: Whisperer server-side estimate now preserves uploaded file context.** The estimate-page flow now keeps the upload token across steps so users can proceed to transcription without re-uploading and without "No file selected" errors.
