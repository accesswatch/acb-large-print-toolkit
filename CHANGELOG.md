...
- **Create Account UI Hidden When User Login Disabled:** Sidebar `Create Account` link in `templates/base.html` and `Need a new account?` link on `templates/auth/login.html` are now hidden when the `GLOW_ENABLE_USER_LOGIN` feature flag is off, since `auth.register` and `firebase_login(createAccount=true)` already reject those flows server-side. Administrator onboarding is unaffected because admins are seeded out-of-band via `SUPER_ADMIN_BOOTSTRAP_EMAILS` / `ADMIN_BOOTSTRAP_EMAILS` and sign in through `GLOW_ENABLE_ADMIN_LOGIN`. Context processor in `app.py` now exposes `user_login_enabled` and `admin_login_enabled` to all templates.
