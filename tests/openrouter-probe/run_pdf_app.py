...
def main() -> None:
    ...
    out_dir = PROBE_ROOT / "pdf"
    pdf_path = PROBE_ROOT / "vision_text.pdf"
    rendered_path = out_dir / "rendered_first_page.png"
    ...
    prompt = "Extract all readable text from this rendered PDF page image."
    ...
    resp = requests.post(
        CHAT_URL,
        headers=request_headers(api_key, "GLOW PDF Probe", content_type_json=True),
        data=json.dumps(payload),
        timeout=60,
    )
    write_json(response_path, response_payload(resp))
    if not resp.ok:
        raise RuntimeError(f"PDF probe failed: HTTP {resp.status_code}")

    actual = resp.json()["choices"][0]["message"]["content"].strip()
    write_text(actual_path, actual + "\n")

    print(f"Input PDF: {pdf_path}")
    print(f"Rendered page image: {rendered_path}")
