(function () {
    ...
    var preferencesForm = document.getElementById('ai-preferences-form');
    ...
    function updateExtendButtonLabels(minutes) {
        Array.prototype.forEach.call(document.querySelectorAll('.provider-session-extend-btn'), function (button) {
            var card = button.closest('[data-provider-card]');
            var providerHeading = card && card.querySelector('h3');
            var label = providerHeading ? providerHeading.textContent.trim() : 'provider';
            button.textContent = 'Keep ' + label + ' active for another ' + minutes + ' minutes';
        });
    }

    function savePreferences(event) {
        var prefStatus = document.getElementById('ai-preferences-status');
        var prefButton = document.getElementById('save-preferences-btn');
        var fd = new FormData(preferencesForm);

        event.preventDefault();
        prefButton.disabled = true;
        prefButton.textContent = 'Saving...';

        fetch(preferencesForm.dataset.saveUrl, {
            method: 'POST',
            body: fd,
            credentials: 'same-origin',
            headers: buildHeaders()
        })
            .then(function (response) { return parseJsonResponse(response, 'save-preferences'); })
            .then(function (data) {
                if (!data.ok) {
                    prefStatus.textContent = 'Could not save: ' + (data.error || 'Unknown error.');
                    return;
                }
                if (data.runtime && data.runtime.extend_minutes) {
                    updateExtendButtonLabels(data.runtime.extend_minutes);
                }
                prefStatus.textContent = 'Saved. New keys last ' + data.runtime.session_minutes + ' minutes and each extension adds ' + data.runtime.extend_minutes + ' minutes.';
            })
            .catch(function (error) {
                prefStatus.textContent = handleRequestFailure('save-preferences', error, 'Could not save AI session and prompt settings.');
            })
            .finally(function () {
                prefButton.disabled = false;
                prefButton.textContent = 'Save session and prompt settings';
            });
    }
