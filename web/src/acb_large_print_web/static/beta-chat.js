(function () {
    ...
    var estimateBox = document.getElementById('playground-estimate');
    ...
    function renderQuota(quota) {
        ...
        parts.push('Session usage: ' + used + '/' + limit + ' chat turns today' + (pct >= 80 ? ' (near limit)' : ''));
        if (quota.session_quota_enabled) {
            parts.push('Rolling requests left: ' + Number(quota.session_requests_remaining || 0));
            if (quota.session_reset_seconds) {
                parts.push('Resets in ' + Math.max(1, Math.round(Number(quota.session_reset_seconds) / 60)) + 'm');
            }
        }
        quotaBox.textContent = parts.join(' | ');
    }
    ...
    function renderEstimate(estimate, quota) {
        if (!estimateBox) {
            return;
        }
        if (!estimate || !textarea.value.trim()) {
            estimateBox.textContent = 'Type a message to preview estimated tokens and cost.';
            return;
        }
        var parts = [
            'Estimated tokens: ' + Number(estimate.estimated_total_tokens || 0)
        ];
        if (estimate.cost_available && estimate.estimated_cost_usd != null) {
            parts.push('Estimated cost: $' + Number(estimate.estimated_cost_usd).toFixed(4));
        } else {
            parts.push('Estimated cost unavailable for this provider/model');
        }
        if (quota && quota.budget_remaining_usd != null) {
            parts.push('Budget left: $' + Number(quota.budget_remaining_usd).toFixed(2));
        }
        estimateBox.textContent = parts.join(' | ');
    }

    function fetchEstimate() {
        if (!form.dataset.estimateUrl || !estimateBox) {
            return;
        }
        if (!textarea.value.trim()) {
            renderEstimate(null, null);
            return;
        }
        fetch(form.dataset.estimateUrl, {
            method: 'POST',
            ...
            body: JSON.stringify({
                ...
                conversation_messages: log.querySelectorAll('.message').length
            })
        })
            .then(function (response) { return response.json(); })
            .then(function (payload) {
                if (payload && payload.ok) {
                    renderEstimate(payload.estimate, payload.quota);
                }
            })
            .catch(function () { });
    }
    ...
    function queueEstimate() {
        ...
        estimateTimer = window.setTimeout(fetchEstimate, 250);
    }
